...
const ShareDropdown = ({ handleClick }: Args) => {
  ...
  useEffect(() => {
    const _handleOutsideClick = (e) => {
      if (!(triggerEl.current.contains(e.target) || e.target === triggerEl.current)) {
        setIsOpen(false);
      }
    }
  ...
  return (
    <div className={css(styles.shareDropdown)}>
      <div className={css(styles.trigger)} onClick={() => setIsOpen(!isOpen)} ref={triggerEl}>
        {icons.shareAlt}
      </div>
      {isOpen &&
        <div className={css(styles.dropdown)}>
          {options.map(opt => (
            <div className={css(styles.opt)} key={opt.value} onClick={() => handleClick(opt)}>
