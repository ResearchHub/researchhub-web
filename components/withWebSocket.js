...
export default function withWebSocket(
  ...
  return (props) => {
    ...
    function listen() {
      ...
      ws.onclose = () => {
        setConnected(false);
        localWarnLog(`Disconnected from websocket at ${url}`);
    ...
    function reconnect() {
      if (!ws || ws.readyState === WebSocket.CLOSED) {
        localWarnLog(`Attempting to reconnect to websocket at ${url}`);
        configureWebSocket();
      }
    }
    ...
    function stopListening(code = null, reason = null) {
      if (ws) {
        ...
        ws.onclose = () => {
          localWarnLog(`Closing websocket connection at ${url}: ${reason}`);
        };
        try {
