...
const ReputationGauge = ({ reputation }: { reputation: Reputation }) => {
  ...
  const userBinPos = reputation.bins.findIndex(bin => reputation.score >= bin[0] && reputation.score <= bin[1]);
      ...
      {reputation.bins.map((bin, index) => {
        ...
        return (
          <div className={css(styles.bin)} key={`bin-` + index}>
            <div className={css(styles.binFilled)} style={{ width: `${binShadePct * 100}%` }} />
          </div>
        )
      })}
    </div>
