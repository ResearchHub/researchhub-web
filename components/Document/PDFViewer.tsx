...
import { faMaximize, faXmark, faMinus, faPlus } from "@fortawesome/pro-light-svg-icons";
...
const PDFViewer = ({ pdfUrl, maxWidth = 900 }: Props) => {
  ...
  const fullScreenViewer = useMemo(() => {
    return (
      <div className={css(styles.expandedWrapper, isExpanded && styles.expandedOn)}>
        <div className={css(styles.expandedNav)}>
          <div onClick={() => setIsExpanded(false)} className={css(styles.closeBtn)}>
            <IconButton>
          ...
          <div style={{ display: "flex"}}>
            ...
            >
              <IconButton>
                ...
                <FontAwesomeIcon icon={faChevronDown} style={{ fontSize: 24 }} />
              </IconButton>
            </GenericMenu>
          </div>
        </div>
        <div style={{ overflowY: "scroll", height: "100vh" }}>
        <_PDFViewer pdfUrl={pdfUrl} viewerWidth={viewerWidth*selectedZoom} onLoadSuccess={onLoadSuccess} onLoadError={onLoadError} showWhenLoading={<DocumentPlaceholder />} />
        </div>
      </div>
    )
  ...
  return (
    <div className={css(styles.container)}> 
      ...
      <div className={css(styles.stickyDocNav)}>
        ...
        <div style={{ borderRight: `2px solid ${colors.BLACK(0.5)}`, height: "20px", marginTop: 5 }}></div>
        <div onClick={() => setIsExpanded(true)} style={{}}>
          <IconButton tooltip="Fullscreen mode">
      ...
      <_PDFViewer pdfUrl={pdfUrl} viewerWidth={viewerWidth} onLoadSuccess={onLoadSuccess} onLoadError={onLoadError} showWhenLoading={<DocumentPlaceholder />} />
    </div>
  )
