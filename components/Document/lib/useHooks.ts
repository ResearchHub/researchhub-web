...
export const useDocument = ({ rawDocumentData, documentType }): [GenericDocument|null, Function] => {
  const [document, setDocument] = useState<null|GenericDocument>(null);
  
  useEffect(() => {
    if (rawDocumentData && isEmpty(document)) {
      const _document = getDocumentFromRaw({ raw: rawDocumentData, type: documentType });
  ...
  if (isEmpty(parsedDocument) && rawDocumentData) {
    parsedDocument = getDocumentFromRaw({ raw: rawDocumentData, type: documentType });
  }
...
export const useDocumentMetadata = ({ rawMetadata, unifiedDocumentId }): [DocumentMetadata | null, Function] => {
