...
export const tabs:Array<{label: string, value: string, isSelected?: boolean}> = [{
  label: "Paper",
  value: "",
},{
  label: "Conversation",
...
export const getTabs = ({ router }: { router: NextRouter }) => {
  ...
  const tabsWithSelected = tabs.map(tab => tab.value === tabName ? { ...tab, isSelected: true } : tab );
  const tabsWithHref = tabsWithSelected.map(tab => ({ ...tab, href: basePath + `/${tab.value}` }));
