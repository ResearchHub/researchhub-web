...
const PaperVersionAuthorsAndMetadataStep = ({
  ...
  const [authorsAndAffiliations, setAuthorsAndAffiliations] = useState<Array<{
  ...
  return (
    <div>
      <div className={css(styles.authorFormWrapper)}>
        {isFormVisible && <AuthorAndAffiliationForm onAffiliationAdded={onAffiliationAdded} handleCloseForm={() => setIsFormVisible(false)} />}
      </div>
      <div className={css(styles.authorsList)}>
        {authorsAndAffiliations.map((authorAndAffiliation, index) => (
          <div key={authorAndAffiliation.author.id} className={css(styles.authorItem)}>
            <div className={css(styles.authorContent)}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                >
                  {isEmpty(authorAndAffiliation.author.profileImage) && (authorAndAffiliation.author.fullName || "")[0]}
                </Avatar>                
              ...
              <FontAwesomeIcon icon={faBuildingColumns} style={{ width: 24 }} />
            ...
            <div className={css(styles.arrowControls)}>
              <button 
                className={css(styles.arrowButton, index === 0 && styles.arrowButtonDisabled)} 
              ...
              <button 
                className={css(styles.arrowButton, index === authorsAndAffiliations.length - 1 && styles.arrowButtonDisabled)} 
